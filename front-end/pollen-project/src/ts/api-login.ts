...
if (form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const emailInput = form.querySelector('[name="login-email"]') as HTMLInputElement;
    const senhaInput = form.querySelector('[name="login-password"]') as HTMLInputElement;

    if (!emailInput || !senhaInput) {
      console.error('❌ Campos de email ou senha não encontrados');
      mensagemDiv.textContent = 'Erro: Formulário inválido.';
